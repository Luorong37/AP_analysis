...
%     complete pipeline.
%   - dual3_control.preset='analysis_only' reads ROI, traces, accepted voltage
%     peaks, and stimulus context from dual3_control.source_results_path,
%     keeps that source folder read-only, and writes downstream analyses to
%     a new timestamped folder.

% Workflow control is passed only through dual3_control. Dual_analysis3_rec
...
% sections below read only this final plan; they do not branch on preset.
preset_full = struct( ...
    'input', "run", ...
    'motion', "run", ...
    'registration', "run", ...
...
analysis_mode = lower(string(dual3_control_effective.preset));
switch analysis_mode
    case "full"
        execution_plan = preset_full;
    case "analysis_only"
        execution_plan = preset_analysis_only;
    case "motion_only"
        execution_plan = preset_motion_only;
    otherwise
        error('Unknown dual3_control.preset: %s', analysis_mode);
end

section_overrides = dual3_control_effective.sections;
if ~isstruct(section_overrides) || ~isscalar(section_overrides)
    error('dual3_control.sections must be one scalar struct.');
end
section_names = fieldnames(execution_plan);
override_names = fieldnames(section_overrides);
unknown_sections = setdiff(override_names, section_names);
if ~isempty(unknown_sections)
    error('Unknown dual3_control.sections field(s): %s', strjoin(unknown_sections, ', '));
end
for section_idx = 1:numel(override_names)
    section_name = override_names{section_idx};
    execution_plan.(section_name) = lower(string(section_overrides.(section_name)));
end

valid_actions = ["run", "reuse", "skip"];
for section_idx = 1:numel(section_names)
    section_name = section_names{section_idx};
    action = string(execution_plan.(section_name));
    if ~isscalar(action) || ~ismember(action, valid_actions)
        error('Section %s has invalid action %s. Use run, reuse, or skip.', section_name, action);
    end
end

% Dependencies are validated before any file is loaded or output is made.
% A run result invalidates reuse of every dependent downstream result.
dependency_edges = {
    'motion',       {'trace'};
...
for edge_idx = 1:size(dependency_edges, 1)
    ...
    if execution_plan.(upstream) == "run"
        for downstream_idx = 1:numel(downstream_list)
            downstream = downstream_list{downstream_idx};
            if execution_plan.(downstream) == "reuse"
                error(['Invalid execution plan: %s="run" invalidates %s="reuse". ' ...
                    'Set %s to "run" or "skip".'], upstream, downstream, downstream);
            end
        end
    end
end
trace_available = ismember(execution_plan.trace, ["run", "reuse"]);
roi_available = ismember(execution_plan.roi, ["run", "reuse"]);
if execution_plan.motion == "run" && execution_plan.input ~= "run"
    error('motion="run" requires input="run".');
end
if execution_plan.registration == "run" && execution_plan.input ~= "run"
    error('registration="run" requires input="run".');
end
if execution_plan.registration == "reuse" && execution_plan.input ~= "reuse"
    error('registration="reuse" requires input="reuse". Use registration="skip" when reloading raw inputs.');
end
if execution_plan.roi == "run" && execution_plan.input ~= "run"
    error('roi="run" requires input="run".');
end
if execution_plan.trace == "run" && ~(execution_plan.input == "run" && roi_available)
    error('trace="run" requires input="run" and roi="run" or "reuse".');
end
if execution_plan.trace == "reuse" && execution_plan.roi ~= "reuse"
    error('trace="reuse" requires roi="reuse" so ROI identity and trace provenance stay together.');
end
for source_section = ["trace", "peak", "visualization", "comparison", "stim", "frequency"]
    if execution_plan.(source_section) == "reuse" && execution_plan.input ~= "reuse"
        error('%s="reuse" requires input="reuse" so the source containers are initialized atomically.', source_section);
    end
end
if execution_plan.peak ~= "skip" && ~trace_available
    error('peak requires trace="run" or "reuse".');
end
for dependent_name = ["visualization", "comparison", "stim", "frequency"]
    if execution_plan.(dependent_name) == "run" && ~trace_available
        error('%s="run" requires trace="run" or "reuse".', dependent_name);
    end
end
if execution_plan.motion == "reuse"
    error(['motion="reuse" is unavailable because the existing result format does not save ' ...
        'the full corrected movies. Use motion="run" or "skip".']);
end
if any(structfun(@(value) string(value) == "reuse", execution_plan)) ...
        && strlength(string(dual3_control_effective.source_results_path)) == 0
    error('At least one section is set to reuse; set dual3_control.source_results_path.');
end
if execution_plan.export == "reuse"
    error('export="reuse" is unsupported. Use export="run" or "skip".');
end
if any(structfun(@(value) string(value) == "reuse", execution_plan))
    ...
    if execution_plan.visualization == "reuse" || execution_plan.comparison == "reuse"
        reuse_required_files(end+1, 1) = "dual_results.mat";
    end
    ...
    validate_dual3_reuse_source(execution_plan, source_path_for_validation);
end
if execution_plan.input == "skip"
    non_input_actions = rmfield(execution_plan, 'input');
    if any(structfun(@(value) string(value) ~= "skip", non_input_actions))
        error('input="skip" is only legal when every other section is also "skip".');
    end
end
fprintf('[Control] Section dependency and reuse-source checks passed.\n');
print_dual3_execution_plan(execution_plan, dual3_control_effective.backend);
dual3_control_effective.sections = execution_plan;

cycle_path = char(string(dual3_control_effective.cycle_path));
analysis_mode = char(analysis_mode);
analysis_backend = char(string(dual3_control_effective.backend));
...
run_motion_correction = execution_plan.motion == "run";
reuse_roi_file = '';
...
if isempty(analysis_run_name)
    if strcmpi(string(analysis_mode), "analysis_only")
...
if ~exist('voltage_peak_polarity_mode', 'var') || isempty(voltage_peak_polarity_mode)
    voltage_peak_polarity_mode = "auto"; % "auto", "positive", or "negative"
end
...
if motion_only_mode && volpy_reanalysis_mode
    error('dual3_control.preset=motion_only requires dual3_control.backend=default.');
end
if motion_only_mode && ~logical(run_motion_correction)
    error('dual3_control.preset=motion_only requires motion="run".');
end
...
parallel_pool_info = struct( ...
    'requested', logical(gpu && ~volpy_reanalysis_mode && execution_plan.input == "run"), ...
    'ready', false, ...
    'num_workers', 0, ...
    'message', "", ...
...
stim_windows = struct();
if stim_context.supported
    if execution_plan.input == "reuse" && isstruct(stim_windows) && ~isempty(fieldnames(stim_windows))
        fprintf('Reusing saved stimulus windows from source input context.\n');
    elseif execution_plan.input == "reuse" && ~(isfield(stim_context, 'logs') && isstruct(stim_context.logs))
        fprintf(['Saved stim windows are unavailable and saved stim_context has no logs. ' ...
...
function print_dual3_execution_plan(execution_plan, backend)
backend = lower(string(backend));
rows = strings(0, 3);
section_names = fieldnames(execution_plan);
for idx = 1:numel(section_names)
    section_name = section_names{idx};
    action = upper(string(execution_plan.(section_name)));
    rows(end+1, :) = [string(section_name), action, section_action_meaning_dual(section_name, action)];
end
if backend == "volpy_voltage_reanalysis"
    rows(end+1, :) = ["Voltage backend", "REPLACE", "Use VolPy voltage traces; calcium/ROI/stimulus context are reused"];
end

fprintf('Dual_analysis3 execution plan:\n');
...
for idx = 1:numel(section_names)
    section_name = section_names{idx};
    action = upper(string(execution_plan.(section_name)));
    fprintf(fid, '%-20s %-10s %s\n', section_name, action, ...
        section_action_meaning_dual(section_name, action));
end
clear cleanup;
...
if use_independent_calcium_motion
    ...
    if cfg.auto_reuse_previous_shift
    ...
    if calcium_auto_reused_previous_shift
...
if use_independent_calcium_motion
    ...
    calcium_motion_info.use_saved_shift = calcium_auto_reused_previous_shift;
...
if params.run_manual_edit
    [peak_table_final, edit_history] = edit_voltage_sensitivity_peaks( ...
...
for roi_idx = 1:nrois
    ...
    polarity = choose_voltage_peak_polarity(trace_i, params.polarity_mode);
...
function [peak_table, edit_history] = edit_voltage_sensitivity_peaks( ...
...
title(ax, sprintf('ROI %d | %s | A add, D delete, N/Space next, R reset, Q quit', ...
    roi_idx, upper(mode)));
xlabel(ax, 'Frame');
...
function polarity = choose_voltage_peak_polarity(trace_i, mode)
...
function run_dual_post_trace_sections( ...
    ...
    nframes_voltage, nframes_calcium, ...
    voltage_polarity, calcium_polarity, calcium_smoothing_window, ...
    voltage_peak_params, reuse_saved_stim_windows)
[voltage_results, ~] = run_voltage_peak_detection_section( ...
    voltage_results, voltage_results_path, save_path, freq_voltage, voltage_peak_params, false, '');
