...
if isempty(pool) || ~contains(class(pool), 'ProcessPool')
    error('remove_background:MissingProcessPool', ...
        'A ProcessPool must be started before calling remove_background.');
end
q = parallel.pool.DataQueue;
n_done = 0;
progress_t0 = tic;
afterEach(q, @report_progress);
background_mask_cells = cell(num_rois, 1);
...
parfor i = 1:num_rois
    ...
    % 更新背景掩码
    background_mask_cells{i} = bg_mask;
...
for i = 1:num_rois
    background_mask(background_mask_cells{i}) = i;
end
fprintf('Background removed\n')

    function report_progress(~)
        n_done = n_done + 1;
        elapsed_s = toc(progress_t0);
        remaining_s = elapsed_s / n_done * (num_rois - n_done);
        fprintf('Background correction %d/%d ROIs (%d%%) | elapsed %.1f s | ETA %.1f s\n', ...
            n_done, num_rois, floor(n_done / num_rois * 100), elapsed_s, remaining_s);
    end
end
