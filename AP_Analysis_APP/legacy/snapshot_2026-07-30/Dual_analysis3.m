...
%   - Motion correction normally uses voltage-estimated shifts for both
%     channels. If frame counts differ, calcium is corrected independently
%     and its own shifts/parameters are saved under calcium-specific names.
%   - Main output files are dual_info.mat, voltage_results.mat,
%     calcium_results.mat, voltage_peak_results.mat, dual_results.mat,
%     stim_results.mat, and -1_explicit_dual_results.mat.
%   - dual3_control.preset='full' creates a new timestamped folder and runs the
%     complete pipeline.
%   - dual3_control.preset='reuse_motion'/'reuse_trace'/'skip_motion' starts
%     from a named section plan, then applies dual3_control.sections overlays.
%   - dual3_control.preset='custom' requires every section action explicitly.
%   - dual3_control.preset='analysis_only' skips motion/registration/ROI,
%     reads traces, accepted voltage peaks, and stimulus context from
%     dual3_control.source_results_path, keeps that source folder read-only,
%     and writes downstream analyses to a new timestamped folder.

% Workflow control is passed only through dual3_control. run_Dual_analysis3_rec
% constructs this struct explicitly for each Cycle run.

%% Helper / Readme
% 1. Set dual3_control.cycle_path to one rebuilt acquisition cycle folder.
% 2. Confirm camera_cfg matches your real camera-role mapping.
% 3. Only change transpose flags here; later sections inherit them.
% 4. Use dual3_control.sections plus source_results_path for saved results.
%    Motion itself cannot be reused because corrected movies are not saved.
% 5. This script includes AP3-style voltage sensitivity peak detection and
%    manual add/delete editing. AP waveform morphology summaries remain in
%    AP_analysis3.
...
% 7. Workflow behavior is controlled only by dual3_control. Unknown fields
%    are rejected so spelling mistakes cannot silently select a preset.
%
...
%   dual3_control.sections.motion = "skip";
...
%   dual3_control.preset = "full";
%   dual3_control.cycle_path = '...\Rec1_...\Cycle1';
%
% Or reuse one previous result folder without rerunning movie/ROI/trace
% processing:
%
%   dual3_control = struct();
%   dual3_control.preset = "analysis_only";
%   dual3_control.cycle_path = '...\Rec1_...\Cycle1';
%   dual3_control.source_results_path = '...\Dual_analysis3\previous_run';
%
% dual3_control is the only workflow-control input. Missing fields use the
% preset below; unrelated workspace variables are ignored.
% Signal-processing parameters remain in the Algorithm Parameters section
% below and are not changed by the full/analysis_only preset.
...
% -------------------------------------------------------------------------
% dual3_control fields:
%
%   cycle_path
%       One Rec*/Cycle* folder to analyze. Raw/rebuilt camera inputs,
%       manifests, and stimulus logs are resolved relative to this folder.
%
%   preset
%       Selects the base section plan before sections overlays are applied.
%       "full"          runs the complete pipeline from movie input.
%       "reuse_motion"  loads current movies and applies saved motion shifts.
%       "reuse_trace"   reuses saved traces and reruns peaks/stim analysis.
%       "skip_motion"   loads movies but skips motion and registration.
%       "analysis_only" reuses saved traces/peaks and reruns later analyses.
%       "motion_only"   stops after motion correction outputs are saved.
%       "custom"        requires all section actions explicitly.
%
%   sections
%       Scalar overlay struct whose named fields override the selected preset.
%       Example: struct('trace',"reuse",'peak',"run",'stim',"run").
%       Every section action must be "run", "reuse", or "skip":
%         run   = execute this section and save a new result.
%         reuse = load this section's saved result from source_results_path.
%         skip  = neither execute nor load this section.
%       Illegal dependency combinations are rejected before output creation.
%
%   run_name
%       Name of the new result folder. Empty creates the standard name:
%       <Rec name>_<Cycle name>_<timestamp>.
%
%   output_path
%       Full destination folder. Empty writes to:
%       <Cycle>\Dual_analysis3\<run_name>.
%       The destination must be new or empty; old and new results are never
%       mixed in one nonempty folder.
%
%   source_results_path
%       Previous Dual_analysis3 result folder used by "reuse" sections.
%       Required when a reused section needs saved results from that folder.
%       For roi="reuse", roi_path below takes priority and can be used
%       without source_results_path. The source remains read-only; reused
%       and newly calculated results are written to output_path.
%
%   roi_path
%       Folder containing a saved ROI MAT file. It is used only when
%       roi="reuse" and takes priority over source_results_path. The folder
%       is searched for Dual3 1_dual_roi_results.mat, AP3 1_raw_ROI.mat,
%       roi.mat, then another valid *ROI*.mat file. This lets one Cycle
%       reuse an ROI stored in another Cycle without changing the source
%       for other reused sections.
%
%   offset_mode
%       Inter-camera registration method used only when registration="run":
%       "none" uses reuse_offset, "manual_points" asks for matching points,
%       and "matlab_register" estimates translation automatically.
%       It is forced to "none" when registration is not run.
%
%   reuse_offset
%       Existing [x y] channel offset used when no new offset is estimated.
%       Empty lets the downstream registration code use its normal default.
%
% Common examples:
%
%   1. Full analysis from movies:
%      dual3_control = struct( ...
%          'cycle_path', 'D:\Data\Rec1_2026-01-01_12-00-00\Cycle1', ...
%          'preset', "full");
%
%   2. Analysis-only using saved traces and accepted peaks:
%      dual3_control = struct( ...
%          'cycle_path', 'D:\Data\Rec1_2026-01-01_12-00-00\Cycle1', ...
...
%
%   3. Reuse traces, rerun peak finding and stimulus analysis:
%      dual3_control = struct( ...
%          'cycle_path', 'D:\Data\Rec1_2026-01-01_12-00-00\Cycle1', ...
%          'preset', "reuse_motion", ...
%          'source_results_path', ...
%              'D:\Data\Rec1_2026-01-01_12-00-00\Cycle1\Dual_analysis3\previous_run', ...
%          'sections', struct('trace',"reuse",'peak',"run",'stim',"run"));
%
%   4. Motion correction only:
%      dual3_control = struct( ...
%          'cycle_path', 'D:\Data\Rec1_2026-01-01_12-00-00\Cycle1', ...
%          'preset', "motion_only");
%
%   5. Custom output folder name:
%      dual3_control = struct( ...
%          'cycle_path', 'D:\Data\Rec1_2026-01-01_12-00-00\Cycle1', ...
%          'preset', "full", ...
%          'run_name', "Rec1_Cycle1_test");
%
%   6. Load current movies but apply shifts from a previous result:
%      dual3_control = struct( ...
%          'cycle_path', 'D:\Data\Rec1_2026-01-01_12-00-00\Cycle1', ...
%          'preset', "full", ...
%          'source_results_path', ...
%              'D:\Data\Rec1_2026-01-01_12-00-00\Cycle1\Dual_analysis3\previous_run', ...
%          'sections', struct('motion',"reuse"));
%
%   7. Reuse an ROI file stored in another Cycle:
%      dual3_control = struct( ...
%          'cycle_path', 'D:\Data\Rec1_2026-01-01_12-00-00\Cycle2', ...
%          'preset', "full", ...
%          'roi_path', ...
%              'D:\Data\Rec1_2026-01-01_12-00-00\Cycle1\Dual_analysis3\previous_run', ...
%          'sections', struct('roi',"reuse"));
%
% Run after defining the control:
%      run('Dual_analysis3.m');
if ~exist('dual3_control', 'var') || isempty(dual3_control)
    dual3_control = struct();
end
dual3_control_defaults = struct( ...
    'cycle_path', '', ...
    'preset', "full", ...
    'sections', struct(), ...
    'run_name', "", ...
    ...
    'reuse_offset', []);
dual3_control_effective = apply_control_defaults_dual(dual3_control, dual3_control_defaults);

% Presets expand into the actual atomic execution plan. The calculation
% sections below read only this final plan; they do not branch on preset.c
%
% Atomic section meanings:
%   input
%       run: load raw/rebuilt camera movies and acquisition metadata.
%       reuse: load saved channel containers and metadata without movies.
%       skip: legal only when every other section is also skipped.
%   motion
%       run: motion-correct both channels and save motion outputs.
%       reuse: load previous shift parameters from source_results_path and
%              apply them to the current raw/rebuilt input movies.
%       skip: do not motion-correct; later sections use their available input.
%   registration
%       run: calculate/apply inter-camera spatial registration.
%       reuse: load saved registration information.
%       skip: do not calculate or load registration.
%   roi
%       run: create/select ROIs for trace extraction.
%       reuse: load the saved ROI result.
%       skip: do not create or load ROI results in this run.
%   trace
%       run: extract and process voltage/calcium traces through SNR.
%       reuse: load saved trace_results from both channel result files.
%       skip: no trace data are made available to downstream run sections.
%   peak
%       run: detect voltage sensitivity peaks and open manual peak editing.
%       reuse: load and validate saved accepted voltage peaks.
%       skip: no accepted peaks are produced for spike-based tuning analysis.
%   visualization
%       run: regenerate trace, map, heatmap, and ROI visualization outputs.
%       reuse: copy the saved visualization result fields.
%       skip: do not generate or load visualization results.
%   comparison
%       run: regenerate voltage-calcium comparison figures/statistics.
%       reuse: copy saved comparison result fields.
%       skip: omit dual-channel comparison.
%   stim
%       run: rebuild stimulus windows and stimulus-specific analyses,
%            including grating tuning curves and DSI/OSI when applicable.
%       reuse: load saved stim_results without recalculating them.
%       skip: omit stimulus-aware calculations.
%   frequency
%       run: regenerate Fourier and wavelet analyses.
%       reuse: load the saved time-frequency result.
%       skip: omit frequency-domain analysis.
%   export
%       run: write the final explicit result bundle.
%       reuse: unsupported.
%       skip: keep modular section files without writing the final bundle.
%
% Preset definitions and dependency validation live in one shared resolver
% used by both Dual_analysis3 and run_Dual_analysis3_rec.
[analysis_mode, preset_sections, overlay_sections, execution_plan, section_resolution] = ...
    resolve_dual3_sections(dual3_control_effective.preset, dual3_control_effective.sections);
dual3_control_effective.requested_preset = section_resolution.requested_preset;
dual3_control_effective.preset = analysis_mode;
dual3_control_effective.preset_sections = preset_sections;
dual3_control_effective.overlay_sections = overlay_sections;
dual3_control_effective.resolved_sections = execution_plan;
dual3_control_effective.sections = overlay_sections;
configured_roi_path = string(dual3_control_effective.roi_path);
if ~isscalar(configured_roi_path)
    error('dual3_control.roi_path must be one folder path.');
end
configured_roi_path = strtrim(configured_roi_path);
if strlength(configured_roi_path) > 0 && execution_plan.roi ~= "reuse"
    error(['dual3_control.roi_path is set, but the ROI section action is "%s". ' ...
        'Set dual3_control.sections.roi="reuse" to load the saved ROI folder.'], ...
        execution_plan.roi);
end
configured_roi_file = "";
source_reuse_plan = execution_plan;
if execution_plan.roi == "reuse" && strlength(configured_roi_path) > 0
    if ~isfolder(configured_roi_path)
        error('roi_path folder does not exist: %s', configured_roi_path);
    end
    configured_roi_file = resolve_roi_file_from_folder(configured_roi_path);
    source_reuse_plan.roi = "skip";
end
reuse_requires_source_results = any(structfun( ...
    @(value) string(value) == "reuse", source_reuse_plan));
if reuse_requires_source_results ...
        && strlength(string(dual3_control_effective.source_results_path)) == 0
    error(['A reused section requires dual3_control.source_results_path. ' ...
        'For roi="reuse" alone, set dual3_control.roi_path instead.']);
end
if reuse_requires_source_results
    source_path_for_validation = char(string(dual3_control_effective.source_results_path));
    if ~isfolder(source_path_for_validation)
        error('source_results_path does not exist: %s', source_path_for_validation);
    end
    reuse_required_files = strings(0, 1);
    if source_reuse_plan.motion == "reuse"
        reused_motion_shift_file = resolve_reused_motion_shift_file( ...
            source_path_for_validation, "voltage");
        if strlength(reused_motion_shift_file) == 0
            error(['motion="reuse" requires shared_motion_shifts_result.mat ' ...
                'or motion_shifts_result.mat in source_results_path.']);
        end
    end
    if source_reuse_plan.input == "reuse"
        reuse_required_files(end+1:end+3, 1) = [ ...
            "dual_info.mat"; "voltage_results.mat"; "calcium_results.mat"];
    end
    if source_reuse_plan.roi == "reuse"
        reuse_required_files(end+1, 1) = "1_dual_roi_results.mat";
        reuse_required_files(end+1, 1) = "dual_results.mat";
    end
    if source_reuse_plan.visualization == "reuse" || source_reuse_plan.comparison == "reuse"
        reuse_required_files(end+1, 1) = "dual_results.mat";
    end
    if source_reuse_plan.stim == "reuse"
        reuse_required_files(end+1, 1) = "stim_results.mat";
    end
    if source_reuse_plan.frequency == "reuse"
        reuse_required_files(end+1, 1) = "8_time_frequency_results.mat";
    end
    reuse_required_files = unique(reuse_required_files, 'stable');
    for required_idx = 1:numel(reuse_required_files)
        required_path = fullfile(source_path_for_validation, reuse_required_files(required_idx));
        if ~isfile(required_path)
            error('Section reuse requires source file: %s', required_path);
        end
    end
    validate_dual3_reuse_source(source_reuse_plan, source_path_for_validation);
end
fprintf('[Control] Section dependency and reuse-source checks passed.\n');
print_dual3_execution_plan(preset_sections, overlay_sections, execution_plan);

cycle_path = char(string(dual3_control_effective.cycle_path));
analysis_mode = char(analysis_mode);
analysis_run_name = char(string(dual3_control_effective.run_name));
save_path = char(string(dual3_control_effective.output_path));
reuse_results_path = char(string(dual3_control_effective.source_results_path));
run_motion_correction = ismember(execution_plan.motion, ["run", "reuse"]);
reuse_roi_file = '';
correct_offset_mode = dual3_control_effective.offset_mode;
reuse_offset = dual3_control_effective.reuse_offset;
if execution_plan.roi == "reuse"
    if strlength(configured_roi_file) > 0
        reuse_roi_file = char(configured_roi_file);
    else
        reuse_roi_file = fullfile(reuse_results_path, '1_dual_roi_results.mat');
    end
end
if execution_plan.registration ~= "run"
    correct_offset_mode = "none";
end
dual3_control_effective.offset_mode = string(correct_offset_mode);

[record_path_for_name, cycle_name_for_name] = fileparts(cycle_path);
[~, record_name_for_name] = fileparts(record_path_for_name);
if isempty(analysis_run_name)
    analysis_run_name = sprintf('%s_%s_%s', ...
        record_name_for_name, cycle_name_for_name, char(nowtime));
end
% -------------------------------------------------------------------------
...
% -------------------------------------------------------------------------
% C. Algorithm Parameters: raw two-folder compatibility
...
% -------------------------------------------------------------------------
% D. Algorithm Parameters: advanced per-camera config
...
% -------------------------------------------------------------------------
% E. Algorithm Parameters: trace processing and peak finding
% -------------------------------------------------------------------------
% Voltage peak polarity:
%   voltage_peak_polarity_mode = "global" follows voltage_polarity for all
%       ROIs unless voltage_peak_roi_polarity provides a per-ROI override.
%   voltage_peak_roi_polarity can be [] or a numeric/cell vector with
%       entries >0 for positive peaks, <0 for negative peaks, and 0/NaN/[]
%       for no per-ROI override.
...
if ~exist('voltage_peak_polarity_mode', 'var') || isempty(voltage_peak_polarity_mode)
    voltage_peak_polarity_mode = "global"; % "global", "auto", "positive", or "negative"
end
if ~exist('voltage_peak_roi_polarity', 'var') || isempty(voltage_peak_roi_polarity)
    voltage_peak_roi_polarity = [];
end
...
if ~exist('voltage_peak_min_prominence_mode', 'var') || isempty(voltage_peak_min_prominence_mode)
    voltage_peak_min_prominence_mode = "relative_factor"; % "relative_factor" or "absolute"
end
...
voltage_peak_params = struct( ...
    ...
    'global_polarity', normalize_voltage_peak_polarity_value(voltage_polarity, 'voltage_polarity'), ...
    'roi_polarity', {voltage_peak_roi_polarity}, ...
...
if ~isfield(motion_cfg, 'saved_calcium_shift_file') || isempty(motion_cfg.saved_calcium_shift_file)
    motion_cfg.saved_calcium_shift_file = '';
end
if ~isfield(motion_cfg, 'highpass') || isempty(motion_cfg.highpass)
    motion_cfg.highpass = true;
end
if ~isfield(motion_cfg, 'auto_reuse_previous_shift') || isempty(motion_cfg.auto_reuse_previous_shift)
    motion_cfg.auto_reuse_previous_shift = false;
end
motion_cfg.enabled = logical(run_motion_correction);
if execution_plan.motion == "reuse"
    motion_cfg.use_saved_shift = true;
    motion_cfg.saved_shift_file = char(resolve_reused_motion_shift_file(reuse_results_path, "voltage"));
    motion_cfg.saved_calcium_shift_file = char(resolve_reused_motion_shift_file(reuse_results_path, "calcium"));
    motion_cfg.auto_reuse_previous_shift = false;
elseif execution_plan.motion == "run"
    motion_cfg.use_saved_shift = false;
    motion_cfg.saved_shift_file = '';
    motion_cfg.saved_calcium_shift_file = '';
    motion_cfg.auto_reuse_previous_shift = false;
...
analysis_only_mode = execution_plan.input == "reuse";
motion_only_mode = strcmpi(string(analysis_mode), "motion_only");
if motion_only_mode && ~logical(run_motion_correction)
    error('dual3_control.preset=motion_only requires motion="run" or "reuse".');
end
...
dual3_control_effective.run_name = string(analysis_run_name);
dual3_control_effective.output_path = string(save_path);
if any(structfun(@(value) string(value) == "reuse", execution_plan)) ...
        && paths_equal_dual(save_path, reuse_results_path)
    error(['A plan containing reuse requires a separate output folder. ' ...
        'The source result folder is read-only and cannot also be save_path.']);
end
if isfolder(save_path)
    existing_output_entries = dir(save_path);
    existing_output_entries = existing_output_entries( ...
        ~ismember({existing_output_entries.name}, {'.', '..'}));
    if ~isempty(existing_output_entries)
        error(['Output folder already exists and is not empty: %s\n' ...
            'Choose a new output_path or run_name so old and new results cannot mix.'], save_path);
    end
else
    mkdir(save_path);
end
fprintf('Effective Dual_analysis3 workflow control:\n');
fprintf('  preset=%s\n', string(analysis_mode));
fprintf('  cycle_path=%s\n', string(cycle_path));
fprintf('  output=%s\n', string(save_path));
fprintf('  run_motion=%d\n', run_motion_correction);
fprintf('  offset_mode=%s\n', string(correct_offset_mode));
if reuse_requires_source_results
    fprintf('  source_results=%s\n', string(reuse_results_path));
end
if execution_plan.roi == "reuse"
    fprintf('  ROI source file=%s\n', string(reuse_roi_file));
end
write_dual3_workflow_log( ...
    fullfile(save_path, 'Dual_analysis3_workflow.log'), ...
    dual3_control_effective, execution_plan, reuse_roi_file);
if execution_plan.input == "skip"
    dual_info = struct( ...
        'analysis_name', 'Dual_analysis3', ...
        'cycle_path', cycle_path, ...
        'save_path', save_path, ...
        'workflow_control', dual3_control_effective, ...
        'created_at', datetime("now"));
    save(fullfile(save_path, 'dual_info.mat'), 'dual_info');
    fprintf('All sections are skipped. Workflow record saved; no analysis was run.\n');
    return;
end
...
parallel_pool_info = struct( ...
    'requested', logical(gpu && execution_plan.input == "run"), ...
    'ready', false, ...
    'num_workers', 0, ...
    'message', "", ...
...
if execution_plan.input == "reuse"
    [source_dual_info, source_voltage_results, source_calcium_results, ...
        source_dual_results, source_stim_results, stim_context, stim_windows] = ...
        load_saved_analysis_only_context(reuse_results_path, execution_plan);
    [dual_info, voltage_results, calcium_results, dual_results, stim_results, time_frequency_results] = ...
        initialize_atomic_reuse_output( ...
        save_path, reuse_results_path, execution_plan, dual3_control_effective, ...
        source_dual_info, source_voltage_results, source_calcium_results, ...
        source_dual_results, source_stim_results);
end

if execution_plan.input == "run"
...
dual_info.workflow_control = dual3_control_effective;
...
%% Motion Correction
% Motion is normally estimated once on voltage and then applied to calcium.
% When frame counts differ, one shift cannot be applied frame-for-frame, so
% calcium falls back to an independently estimated rigid correction.
print_section('Motion Correction');
fprintf('Motion section action: %s | enabled: %d\n', ...
    upper(string(execution_plan.motion)), logical(run_motion_correction));
if run_motion_correction
    if execution_plan.motion == "reuse"
        fprintf('Applying saved motion shifts from: %s\n', string(reuse_results_path));
    end
    if nframes_voltage == nframes_calcium
        ...
        fprintf('Frame-count mismatch detected: voltage=%d, calcium=%d.\n', nframes_voltage, nframes_calcium);
        if execution_plan.motion == "reuse"
            fprintf('Applying the saved voltage and calcium shift fields independently.\n');
        else
            fprintf('Fallback motion model: estimate voltage and calcium rigid NoRMCorre shifts independently.\n');
        end
    end
...
% From here on, non-motion-only runs store movies in [pixels x frames] form
% because the ROI-processing functions work primarily on trace matrices.
...
if voltage_motion_info.applied
    if strcmpi(string(calcium_motion_info.method), "NoRMCorre_rigid_independent_calcium")
        fprintf('Motion complete | voltage shift file unchanged: %s\n', voltage_motion_info.shift_file);
    else
...
if isfield(calcium_motion_info, 'shift_file') && strlength(string(calcium_motion_info.shift_file)) > 0 ...
        && strcmpi(string(calcium_motion_info.method), "NoRMCorre_rigid_independent_calcium")
    fprintf('Saved independent calcium shift file: %s\n', string(calcium_motion_info.shift_file));
    fprintf('Saved independent calcium parameter file: %s\n', string(calcium_motion_info.parameter_file));
    if isfield(calcium_motion_info, 'parameters')
        calcium_options = calcium_motion_info.parameters;
        fprintf(['Calcium NoRMCorre parameters | d1=%d d2=%d bin_width=%d ' ...
            'max_shift=%d us_fac=%d iter=%d correct_bidir=%d\n'], ...
            calcium_options.d1, calcium_options.d2, calcium_options.bin_width, ...
            calcium_options.max_shift, calcium_options.us_fac, ...
            calcium_options.iter, calcium_options.correct_bidir);
    end
end
if isfield(calcium_motion_info, 'metrics_png') && strlength(string(calcium_motion_info.metrics_png)) > 0
    fprintf('Saved calcium motion metrics plot: %s\n', string(calcium_motion_info.metrics_png));
end
if motion_only_mode
    print_section('Motion Only Stop');
    dual_results.motion_only = struct( ...
        'status', "completed", ...
        'analysis_mode', string(analysis_mode), ...
        'save_path', string(save_path), ...
        'voltage_motion_info', voltage_motion_info, ...
        'calcium_motion_info', calcium_motion_info, ...
        'created_at', datetime("now"));
    save(dual_results_path, 'dual_results', '-v7.3');
    fprintf('Motion-only mode completed. Stopping before Channel Registration / ROI / trace analysis.\n');
...
elseif execution_plan.registration == "reuse"
    source_voltage_for_registration = load_required_struct( ...
        fullfile(reuse_results_path, 'voltage_results.mat'), 'voltage_results');
    if ~isfield(source_voltage_for_registration, 'movie_info') ...
            || ~isfield(source_voltage_for_registration.movie_info, 'channel_registration')
        error('registration="reuse" requires saved movie_info.channel_registration.');
    end
    channel_registration_info = source_voltage_for_registration.movie_info.channel_registration;
    roi_offset_mode_for_selection = "none";
    fprintf('Reused channel registration from: %s\n', reuse_results_path);
else
    channel_registration_info = struct( ...
        'method', "skipped", ...
        'offset_xy', [0, 0], ...
        'created_at', datetime("now"));
    roi_offset_mode_for_selection = "none";
    fprintf('Channel registration skipped.\n');
end
...
if ~usejava('desktop') && requires_interactive_roi_session
    error(['Interactive ROI selection requires a MATLAB Desktop session. ' ...
        'Please rerun Dual_analysis3 or run_Dual_analysis3_rec inside an existing MATLAB GUI window, ' ...
        'or set roi="reuse" and source_results_path to a saved Dual_analysis3 result folder.']);
end
...
fprintf('ROI complete | nrois=%d | offset=[%.3f %.3f] | map source=%s | roi file=%s\n', ...
    ...
    fprintf('Atomic reuse input initialized from: %s\n', reuse_results_path);
end

if execution_plan.trace == "skip"
    voltage_results.trace_results = struct();
    calcium_results.trace_results = struct();
    save(voltage_results_path, 'voltage_results', '-v7.3');
    save(calcium_results_path, 'calcium_results', '-v7.3');
end

if execution_plan.trace == "run"
%% Background Removal
...
elseif execution_plan.trace == "reuse"
    fprintf('Trace section reused from source: %s\n', reuse_results_path);
else
    fprintf('Trace section skipped.\n');
end

%% Voltage Sensitivity Peak Detection
% Match the AP_analysis3 sensitivity-peak workflow immediately after SNR:
% automatic findpeaks, optional add/delete editing, and explicit storage of
% both detected and final accepted peak stages.
if execution_plan.peak == "run"
    [voltage_results, ~] = run_voltage_peak_detection_section( ...
        voltage_results, voltage_results_path, save_path, freq_voltage, voltage_peak_params, false, '');
elseif execution_plan.peak == "reuse"
    fprintf('Peak section reused from source: %s\n', reuse_results_path);
else
    fprintf('Peak section skipped.\n');
end

if execution_plan.visualization == "run"
...
elseif execution_plan.visualization == "reuse"
    fprintf('Visualization section reused from source: %s\n', reuse_results_path);
else
    fprintf('Visualization section skipped.\n');
end

if any([execution_plan.comparison, execution_plan.stim, execution_plan.frequency] == "run") ...
        && (~exist('stim_windows', 'var') || ~isstruct(stim_windows) || isempty(fieldnames(stim_windows)))
    has_stim_sync_logs = isfield(stim_context, 'logs') ...
        && isstruct(stim_context.logs) ...
        && isfield(stim_context.logs, 'sync') ...
        && istable(stim_context.logs.sync);
    if stim_context.supported && ~has_stim_sync_logs
        fprintf(['Saved stim_context has no sync logs. ' ...
            'Rebuilding stimulus metadata from the current cycle manifests/logs.\n']);
    ...
    if stim_context.supported && has_stim_sync_logs
        stim_windows = build_visual_stim_windows( ...
            stim_context, freq_voltage, freq_calcium, nframes_voltage, nframes_calcium);
    else
        stim_windows = struct();
        if execution_plan.stim == "run" ...
                && strcmpi(string(stim_context.recordmode), "visualstim")
            error(['stim="run" requires recoverable visual-stim sync logs. ' ...
                'No valid logs.sync table was found in the source result or current cycle.']);
        end
    end
end
...
% preprocessing.
if execution_plan.comparison == "run"
print_section('Dual Comparison');
fprintf('Building dual-channel trace comparison results...\n');
if stim_context.supported
    if isstruct(stim_windows) && ~isempty(fieldnames(stim_windows))
        fprintf('Using prepared stimulus windows for dual comparison.\n');
    end
...
fprintf('  trace comparison uses t_voltage for voltage and t_calcium for calcium\n');
...
dual_results.comparison = struct( ...
    ...
    'info', struct( ...
        ...
        'created_at', datetime("now")));
save(dual_results_path, 'dual_results', '-v7.3');
fprintf('Dual trace comparison saved to: %s\n', dual_results_path);
elseif execution_plan.comparison == "reuse"
    fprintf('Comparison section reused from source: %s\n', reuse_results_path);
else
    fprintf('Comparison section skipped.\n');
end

%% Stim-Aware Analysis
% Shared stimulus-aware analysis after Dual Comparison. This saves the
% stim-shadow trace summary and all grating/condition-response outputs.
if execution_plan.stim == "run"
    stim_results = run_dual_stim_aware_analysis_section( ...
        save_path, ...
...
elseif execution_plan.stim == "reuse"
    fprintf('Stim section reused from source: %s\n', reuse_results_path);
else
    fprintf('Stim section skipped.\n');
end
...
if execution_plan.frequency == "run"
...
time_frequency_record = build_section_record( ...
    ...
    struct(), ...
    "To rerun the ROI-after analysis pipeline, set dual3_control.preset='analysis_only', set dual3_control.source_results_path, and run Dual_analysis3 again.");
...
elseif execution_plan.frequency == "reuse"
    fprintf('Frequency section reused from source: %s\n', reuse_results_path);
else
    time_frequency_results = struct();
    time_frequency_results_path = '';
    fprintf('Frequency section skipped.\n');
end
...
elseif execution_plan.export == "reuse"
    error('export="reuse" is not supported. Use export="run" or "skip".');
else
    fprintf('Export section skipped.\n');
end

fprintf('Dual_analysis3 finished.\n');
fprintf('Results saved to: %s\n', save_path);

function control = apply_control_defaults_dual(control, defaults)
if ~isstruct(control) || ~isscalar(control)
    error('dual3_control must be one scalar struct.');
end
unknown_fields = setdiff(fieldnames(control), fieldnames(defaults));
if ~isempty(unknown_fields)
    error('Unknown dual3_control field(s): %s', strjoin(unknown_fields, ', '));
end
...
function print_dual3_execution_plan(preset_sections, overlay_sections, execution_plan)
rows = strings(0, 5);
section_names = fieldnames(execution_plan);
for idx = 1:numel(section_names)
    section_name = section_names{idx};
    preset_action = get_section_action_dual(preset_sections, section_name, "CUSTOM");
    overlay_action = get_section_action_dual(overlay_sections, section_name, "-");
    resolved_action = upper(string(execution_plan.(section_name)));
    rows(end+1, :) = [string(section_name), preset_action, overlay_action, ...
        resolved_action, section_action_meaning_dual(section_name, resolved_action)];
end

fprintf('Dual_analysis3 section resolution:\n');
fprintf('  %-18s %-10s %-10s %-10s %s\n', ...
    'Section', 'Preset', 'Overlay', 'Resolved', 'Meaning');
for idx = 1:size(rows, 1)
    fprintf('  %-18s %-10s %-10s %-10s %s\n', ...
        rows(idx, 1), rows(idx, 2), rows(idx, 3), rows(idx, 4), rows(idx, 5));
end
end

function action = get_section_action_dual(sections, section_name, missing_value)
...
function meaning = section_action_meaning_dual(section_name, action)
if action == "RUN"
    meaning = "Execute this section and save new results";
elseif action == "REUSE"
    meaning = "Load this section's saved result from source_results_path";
else
    meaning = "Do not execute or load this section";
end
if strcmp(section_name, 'input') && action == "RUN"
    meaning = "Load raw/rebuilt camera inputs";
elseif strcmp(section_name, 'input') && action == "REUSE"
    meaning = "Load saved metadata/context without loading movies";
elseif strcmp(section_name, 'motion') && action == "REUSE"
    meaning = "Load saved shifts and apply them to current input movies";
end
end

function validate_dual3_reuse_source(execution_plan, source_path)
source_dual_info = struct();
source_voltage_results = struct();
source_calcium_results = struct();
source_dual_results = struct();

if execution_plan.input == "reuse"
    source_dual_info = load_required_struct( ...
        fullfile(source_path, 'dual_info.mat'), 'dual_info');
    if ~isfield(source_dual_info, 'stim_context')
        error('input="reuse" requires dual_info.stim_context in source_results_path.');
    end
end
if any([execution_plan.registration, execution_plan.trace, execution_plan.peak] == "reuse")
    source_voltage_results = load_required_struct( ...
        fullfile(source_path, 'voltage_results.mat'), 'voltage_results');
end
if execution_plan.trace == "reuse"
    source_calcium_results = load_required_struct( ...
        fullfile(source_path, 'calcium_results.mat'), 'calcium_results');
    if ~isfield(source_voltage_results, 'trace_results') ...
            || ~isfield(source_calcium_results, 'trace_results')
        error('trace="reuse" requires trace_results in both channel result files.');
    end
    fetch_trace_stage(source_voltage_results, 'sensitivity');
    fetch_trace_stage(source_calcium_results, 'sensitivity');
end
if execution_plan.registration == "reuse"
    if ~isfield(source_voltage_results, 'movie_info') ...
            || ~isfield(source_voltage_results.movie_info, 'channel_registration')
        error('registration="reuse" requires voltage_results.movie_info.channel_registration.');
    end
end
if execution_plan.peak == "reuse"
    if ~isfield(source_voltage_results, 'peak_results') ...
            || isempty(source_voltage_results.peak_results)
        peak_file = fullfile(source_path, 'voltage_peak_results.mat');
        if isfile(peak_file)
            source_voltage_results.peak_results = load_required_struct(peak_file, 'peak_results');
        end
    end
    sensitivity = fetch_trace_stage(source_voltage_results, 'sensitivity');
    [valid_peaks, peak_reason] = validate_saved_voltage_peak_results( ...
        source_voltage_results, size(sensitivity, 2), size(sensitivity, 1));
    if ~valid_peaks
        error('peak="reuse" source validation failed: %s', peak_reason);
    end
end
if any([execution_plan.roi, execution_plan.visualization, execution_plan.comparison] == "reuse")
    source_dual_results = load_required_struct( ...
        fullfile(source_path, 'dual_results.mat'), 'dual_results');
end
if execution_plan.roi == "reuse" && ~isfield(source_dual_results, 'registration')
    error('roi="reuse" requires dual_results.registration.');
end
if execution_plan.visualization == "reuse" && ~isfield(source_dual_results, 'visualizations')
    error('visualization="reuse" requires dual_results.visualizations.');
end
if execution_plan.comparison == "reuse" && ~isfield(source_dual_results, 'comparison')
    error('comparison="reuse" requires dual_results.comparison.');
end
if execution_plan.stim == "reuse"
    source_stim_results = load_required_struct( ...
        fullfile(source_path, 'stim_results.mat'), 'stim_results');
    if isempty(fieldnames(source_stim_results))
        error('stim="reuse" requires nonempty stim_results.');
    end
end
if execution_plan.frequency == "reuse"
    load_required_struct(fullfile(source_path, '8_time_frequency_results.mat'), ...
        'time_frequency_results');
end
fprintf('[Control] Reused section fields validated in source_results_path.\n');
end

function write_dual3_workflow_log(log_path, workflow_control, execution_plan, roi_source_file)
fid = fopen(log_path, 'w');
if fid < 0
    warning('Dual_analysis3:WorkflowLogOpenFailed', ...
        'Could not write workflow log: %s', log_path);
    return;
end
cleanup = onCleanup(@() fclose(fid));
fprintf(fid, 'Dual_analysis3 workflow control\n');
fprintf(fid, 'created_at=%s\n', char(datetime('now', 'Format', 'yyyy-MM-dd HH:mm:ss')));
if isfield(workflow_control, 'requested_preset')
    fprintf(fid, 'requested_preset=%s\n', string(workflow_control.requested_preset));
end
fprintf(fid, 'preset=%s\n', string(workflow_control.preset));
fprintf(fid, 'cycle_path=%s\n', string(workflow_control.cycle_path));
fprintf(fid, 'output_path=%s\n', string(workflow_control.output_path));
fprintf(fid, 'source_results_path=%s\n', string(workflow_control.source_results_path));
fprintf(fid, 'roi_path=%s\n', string(workflow_control.roi_path));
fprintf(fid, 'offset_mode=%s\n', string(workflow_control.offset_mode));
if strlength(string(roi_source_file)) > 0
    fprintf(fid, 'roi_source_file=%s\n', string(roi_source_file));
end
fprintf(fid, 'dependency_validation=passed\n');
fprintf(fid, '\n%-20s %-10s %-10s %-10s %s\n', ...
    'Section', 'Preset', 'Overlay', 'Resolved', 'Meaning');
section_names = fieldnames(execution_plan);
for idx = 1:numel(section_names)
    section_name = section_names{idx};
    preset_action = get_section_action_dual(workflow_control.preset_sections, section_name, "CUSTOM");
    overlay_action = get_section_action_dual(workflow_control.overlay_sections, section_name, "-");
    resolved_action = upper(string(execution_plan.(section_name)));
    fprintf(fid, '%-20s %-10s %-10s %-10s %s\n', ...
        section_name, preset_action, overlay_action, resolved_action, ...
        section_action_meaning_dual(section_name, resolved_action));
end
clear cleanup;
fprintf('Workflow control log saved to: %s\n', log_path);
end
...
elseif isfield(roi_cache, 'mask') && ~isempty(roi_cache.mask)
    mask_voltage = roi_cache.mask;
end
...
function roi_file = resolve_roi_file_from_folder(roi_folder)
roi_folder = char(string(roi_folder));
listing = dir(fullfile(roi_folder, '*.mat'));
if isempty(listing)
    error('roi_path contains no MAT files: %s', roi_folder);
end
...
for priority_idx = 1:numel(priority_names)
    ...
    [valid_roi, reason] = validate_roi_mask_file(candidate);
    if ~valid_roi
        error('ROI file %s is invalid: %s', candidate, reason);
    end
    roi_file = string(candidate);
    fprintf('Resolved ROI file from roi_path:\n  %s\n', roi_file);
    return;
end
...
for candidate_idx = generic_idx(:)'
    ...
    if validate_roi_mask_file(candidate)
        valid_files(end+1, 1) = string(candidate); %#ok<AGROW>
    end
end
if isempty(valid_files)
    error(['No valid ROI MAT file was found directly inside roi_path: %s. ' ...
        'Expected rois.bwmask, bwmask, or mask.'], roi_folder);
end
if numel(valid_files) > 1
    error('Multiple valid ROI MAT files were found in roi_path; specify a folder with one ROI source:\n  %s', ...
...
function [is_valid, reason] = validate_roi_mask_file(roi_file)
is_valid = false;
reason = "";
try
...
% Use voltage shifts for both channels when frame counts match. If they do
% not match, preserve the normal voltage outputs and estimate a separate
% calcium shift field under calcium-specific output names.
...
use_independent_calcium_motion = nframes_v ~= nframes_c;
...
if cfg.use_saved_shift && strlength(shift_source_file) == 0
    error('Saved voltage motion shift file was requested but could not be resolved.');
end
...
if (cfg.use_saved_shift && strlength(shift_source_file) > 0) || auto_reused_previous_shift
    ...
    if numel(shifts_r) ~= nframes_v
        error(['Saved voltage shifts contain %d frames but the current voltage movie has %d. ' ...
            'Use motion="run" to estimate new shifts.'], numel(shifts_r), nframes_v);
    end
...
calcium_shift_res_path = '';
calcium_params_save_path = '';
calcium_shift_source_file = "";
calcium_auto_reused_previous_shift = false;
if use_independent_calcium_motion
    fprintf(['Voltage/calcium frame counts differ (%d vs %d). ' ...
        'Estimating an independent calcium motion model.\n'], nframes_v, nframes_c);
    options_c = NoRMCorreSetParms('d1', ncols_c, 'd2', nrows_c, 'bin_width', 200, ...
        'max_shift', 30, 'us_fac', 30, 'iter', 1, 'correct_bidir', false);
    calcium_shift_res_path = fullfile(save_path, 'calcium_motion_shifts_result.mat');
    calcium_params_save_path = fullfile(save_path, 'calcium_motion_correction_para.mat');

    calcium_shift_source_file = string(cfg.saved_calcium_shift_file);
    if strlength(calcium_shift_source_file) == 0 && cfg.auto_reuse_previous_shift
        calcium_shift_source_file = find_latest_previous_calcium_motion_shift(cycle_path, save_path);
        calcium_auto_reused_previous_shift = strlength(calcium_shift_source_file) > 0;
    end

    reuse_calcium_shift = cfg.use_saved_shift && strlength(calcium_shift_source_file) > 0;
    if cfg.use_saved_shift && ~reuse_calcium_shift
        error(['The channel frame counts differ, so motion="reuse" requires ' ...
            'calcium_motion_shifts_result.mat in source_results_path.']);
    end
    if reuse_calcium_shift || calcium_auto_reused_previous_shift
        fprintf('Reusing existing calcium motion shift file:\n  %s\n', calcium_shift_source_file);
        calcium_shift_data = load(char(calcium_shift_source_file));
        if isfield(calcium_shift_data, 'options_c')
            options_c = calcium_shift_data.options_c;
        elseif isfield(calcium_shift_data, 'options_r')
            options_c = calcium_shift_data.options_r;
        end
        if isfield(calcium_shift_data, 'shifts_c')
            shifts_c = calcium_shift_data.shifts_c;
        elseif isfield(calcium_shift_data, 'shifts_r')
            shifts_c = calcium_shift_data.shifts_r;
        else
            error('Saved calcium motion shift file contains neither shifts_c nor shifts_r: %s', ...
                calcium_shift_source_file);
        end
        if numel(shifts_c) ~= nframes_c
            if cfg.use_saved_shift
                error(['Saved calcium shifts contain %d frames but the current calcium movie has %d. ' ...
                    'Use motion="run" to estimate new shifts.'], numel(shifts_c), nframes_c);
            else
                warning('Dual_analysis3:CalciumSavedShiftFrameMismatch', ...
                    ['Saved calcium shifts contain %d frames but current calcium movie has %d. ' ...
                    'Re-estimating calcium motion.'], numel(shifts_c), nframes_c);
                calcium_auto_reused_previous_shift = false;
                calcium_shift_source_file = "";
            end
        end
    ...
    if ~(reuse_calcium_shift || calcium_auto_reused_previous_shift)
        if cfg.highpass
            calcium_for_estimation = create_temp_highpass(calcium_single);
        else
            calcium_for_estimation = calcium_single;
        end
        [~, shifts_c, ~] = normcorre_batch(calcium_for_estimation, options_c);
        clear calcium_for_estimation;
    end
    save(calcium_shift_res_path, 'shifts_c', 'options_c', '-v7.3');
    calcium_corrected = apply_shifts(calcium_single, shifts_c, options_c);
    calcium_motion_context = struct( ...
        'reason', "voltage_calcium_frame_count_mismatch", ...
        'voltage_frame_count', nframes_v, ...
        'calcium_frame_count', nframes_c, ...
        'source_shift_file', calcium_shift_source_file, ...
        'auto_reused_previous_shift', calcium_auto_reused_previous_shift, ...
        'created_at', datetime("now"));
    save(calcium_params_save_path, 'options_c', 'cfg', 'calcium_motion_context');
else
...
calcium_motion_metrics_file = '';
calcium_motion_metrics_fig = '';
calcium_motion_metrics_png = '';
if use_independent_calcium_motion
    calcium_motion_metrics_file = fullfile(save_path, 'calcium_motion_metrics.mat');
    calcium_motion_metrics_fig = fullfile(save_path, 'calcium_motion_metrics.fig');
    calcium_motion_metrics_png = fullfile(save_path, 'calcium_motion_metrics.png');
    save_shared_motion_metrics( ...
        calcium_single, calcium_corrected, ...
        shifts_c, options_c.max_shift, ...
        calcium_motion_metrics_file, calcium_motion_metrics_fig, ...
        calcium_motion_metrics_png, downsample_factor, 'Calcium');
end
...
if use_independent_calcium_motion
    voltage_motion_info.shared_with_role = '';
    calcium_motion_info.method = 'NoRMCorre_rigid_independent_calcium';
    calcium_motion_info.shift_file = calcium_shift_res_path;
    calcium_motion_info.source_shift_file = char(calcium_shift_source_file);
    calcium_motion_info.parameter_file = calcium_params_save_path;
    calcium_motion_info.metrics_file = calcium_motion_metrics_file;
    calcium_motion_info.metrics_fig = calcium_motion_metrics_fig;
    calcium_motion_info.metrics_png = calcium_motion_metrics_png;
    calcium_motion_info.use_saved_shift = reuse_calcium_shift || calcium_auto_reused_previous_shift;
    calcium_motion_info.auto_reused_previous_shift = calcium_auto_reused_previous_shift;
    calcium_motion_info.source_role = 'calcium';
    calcium_motion_info.frame_count_fallback = true;
    calcium_motion_info.parameters = options_c;
else
...
function save_shared_motion_metrics(full_raw, full_corrected, shifts_r, max_shift, mat_file, fig_file, png_file, metric_downsample_factor, channel_label)
...
if nargin < 9 || strlength(string(channel_label)) == 0
    channel_label = "Shared";
else
    channel_label = string(channel_label);
end
...
if channel_label == "Shared"
    figure_name = 'Shared Motion Metrics';
    shift_title = 'Shared Rigid Shifts';
    correlation_title = 'Correlation Coefficients On Full Voltage Movie';
    metric_description = sprintf('High-pass filtered metrics skipped\nFull-voltage metrics use time downsample x%d', ...
        metric_downsample_factor);
else
    figure_name = sprintf('%s Motion Metrics', channel_label);
    shift_title = sprintf('%s Rigid Shifts', channel_label);
    correlation_title = sprintf('Correlation Coefficients On Full %s Movie', channel_label);
    metric_description = sprintf('High-pass filtered metrics skipped\nFull-%s metrics use time downsample x%d', ...
        lower(channel_label), metric_downsample_factor);
end
...
subplot(3, 1, 2);
text(0.5, 0.5, metric_description, ...
...
function shift_file = resolve_reused_motion_shift_file(source_results_path, channel_role)
shift_file = "";
source_results_path = char(string(source_results_path));
if isempty(source_results_path) || ~isfolder(source_results_path)
    return;
end
if strcmpi(string(channel_role), "calcium")
    candidates = {'calcium_motion_shifts_result.mat'};
else
    candidates = {'shared_motion_shifts_result.mat', 'motion_shifts_result.mat'};
end
for idx = 1:numel(candidates)
    candidate = fullfile(source_results_path, candidates{idx});
    if isfile(candidate)
        shift_file = string(candidate);
        return;
    end
end
end

function shift_file = find_latest_previous_calcium_motion_shift(cycle_path, current_save_path)
shift_file = "";
dual_root = fullfile(cycle_path, 'Dual_analysis3');
if ~isfolder(dual_root)
    return;
end
listing = dir(fullfile(dual_root, '**', 'calcium_motion_shifts_result.mat'));
shift_file = select_latest_shift_candidate(listing, current_save_path);
end
...
if ~isfield(stim_context, 'logs') ...
        || ~isstruct(stim_context.logs) ...
        || ~isfield(stim_context.logs, 'sync') ...
        || ~istable(stim_context.logs.sync)
    error(['Stimulus-window construction requires stim_context.logs.sync. ' ...
        'Reload stimulus metadata from the current cycle before calling this function.']);
end

switch char(stim_context.stim_type)
    case 'visualstim_random_grating'
        stim_windows = build_random_grating_stim_windows(stim_context, freq_voltage, freq_calcium, nframes_voltage, nframes_calcium);
...
function stim_windows = build_random_grating_stim_windows(stim_context, freq_voltage, freq_calcium, nframes_voltage, nframes_calcium)
stim_windows = initialize_stim_windows(stim_context);
sync_table = stim_context.logs.sync;
[orientations, angle_sequence_source] = resolve_random_grating_angle_sequence(stim_context);
anchor_row = 2;
...
ntrials = numel(orientations);
if ntrials < 1
    error('random_drifting_grating has an empty logs.stimulus.angleSequence.');
end
if available_trials < ntrials
    error(['random_drifting_grating has %d labeled trials in %s, but logs.sync only ' ...
        'contains enough frames for %d complete trials.'], ntrials, char(angle_sequence_source), available_trials);
end

unique_orientations = unique(orientations, 'stable');
trial_counts = arrayfun(@(x) sum(orientations == x), unique_orientations);
if isempty(trial_counts) || any(trial_counts ~= trial_counts(1))
    error('random_drifting_grating requires equal repeat counts per direction. Counts: %s.', mat2str(trial_counts));
end
...
trial_labels = string(arrayfun(@(x) sprintf('%.0f deg', x), orientations(:)', 'UniformOutput', false))';
stim_windows = finalize_stim_windows_from_rows( ...
    stim_windows, stim_context, sync_table, voltage_base_rows, voltage_stim_rows, ...
    condition_index(:), condition_labels(:), trial_labels(:), ...
    orientations(:), unique_orientations(:)', ...
    freq_voltage, freq_calcium, nframes_voltage, nframes_calcium, hsv(max(1, numel(unique_orientations))));
stim_windows.is_grating = true;
stim_windows.angle_sequence_source = angle_sequence_source;
stim_windows.angle_sequence = orientations(:)';
stim_windows.ntrials_labeled = ntrials;
stim_windows.ntrials_available = available_trials;
stim_windows.orientation_trial_counts = struct( ...
    'orientations', unique_orientations(:)', ...
    'counts', trial_counts(:)', ...
    'repeat_count', trial_counts(1));
end

function [orientations, source_name] = resolve_random_grating_angle_sequence(stim_context)
source_name = "logs.stimulus.angleSequence";
if ~isfield(stim_context, 'logs') ...
        || ~isstruct(stim_context.logs) ...
        || ~isfield(stim_context.logs, 'stimulus') ...
        || ~isstruct(stim_context.logs.stimulus) ...
        || ~isfield(stim_context.logs.stimulus, 'angleSequence') ...
        || isempty(stim_context.logs.stimulus.angleSequence)
    error('random_drifting_grating requires logs.stimulus.angleSequence.');
end
orientations = double(stim_context.logs.stimulus.angleSequence(:)');
if any(~isfinite(orientations))
    error('random_drifting_grating logs.stimulus.angleSequence contains non-finite values.');
end
end
...
elseif strcmp(selected_program, 'random_drifting_grating')
    stim_type = "visualstim_random_grating";
...
fprintf('  OSI/DSI eligible: %s\n', ternary(any(strcmpi(string(stim_context.stim_type), ["visualstim_grating", "visualstim_random_grating"])), 'yes', 'no'));
...
function [voltage_results, peak_results] = run_voltage_peak_detection_section( ...
    voltage_results, voltage_results_path, save_path, frame_rate, params, ...
    reuse_saved_peak_results, reuse_source_path)
print_section('Voltage Sensitivity Peak Detection');
voltage_sensitivity = fetch_trace_stage(voltage_results, 'sensitivity');
nrois = size(voltage_sensitivity, 2);
peak_results = struct();

if nargin < 6 || isempty(reuse_saved_peak_results)
    reuse_saved_peak_results = false;
end
if nargin < 7
    reuse_source_path = '';
end
if reuse_saved_peak_results
    [can_reuse, reuse_reason] = validate_saved_voltage_peak_results( ...
        voltage_results, nrois, size(voltage_sensitivity, 1));
    if can_reuse
        peak_results = voltage_results.peak_results;
        peak_results.reuse_info = struct( ...
            'reused', true, ...
            'source_results_path', string(reuse_source_path), ...
            'destination_results_path', string(save_path), ...
            'validation', reuse_reason, ...
            'reused_at', datetime("now"));
        accepted_data = peak_results.accepted_for_events.data;
        peaks_index = accepted_data.index;
        peaks_amplitude = accepted_data.amplitude;
        peaks_polarity = accepted_data.polarity;
        save(fullfile(save_path, '5_voltage_accepted_peaks.mat'), ...
            'peaks_index', 'peaks_polarity', 'peaks_amplitude');
        save(fullfile(save_path, 'voltage_peak_results.mat'), 'peak_results', '-v7.3');
        voltage_results.peak_results = peak_results;
        save(voltage_results_path, 'voltage_results', '-v7.3');
        fprintf('Reused saved voltage peaks: %d ROIs | %d accepted peaks\n', ...
            nrois, sum(cellfun(@numel, peaks_index)));
        fprintf('  source: %s\n', string(reuse_source_path));
        return;
    end
    error(['Saved voltage peaks cannot be reused: %s. Set peak="run" to find peaks ' ...
        'again, or use peak="reuse" with a source_results_path containing valid ' ...
        'accepted peaks.'], reuse_reason);
end

fprintf('Detecting voltage peaks from trace_results.sensitivity...\n');
fprintf('Voltage peak polarity mode: %s | global voltage polarity: %d\n', ...
    string(params.polarity_mode), params.global_polarity);
[peak_table_detected, peaks_index, peaks_amplitude, peaks_polarity] = ...
    detect_voltage_sensitivity_peaks(voltage_sensitivity, frame_rate, params);
...
peak_results.sensitivity_detected.info = struct( ...
    ...
    'parameters', rmfield(params, 'run_manual_edit'), ...
    'created_at', datetime("now"));
peak_results.current_result = 'sensitivity_detected';
save(fullfile(save_path, '4_voltage_sensitivity_peak_detect_results.mat'), ...
    'peak_table_detected', 'peaks_index', 'peaks_amplitude', 'peaks_polarity', 'params');

edit_history = struct('action', {}, 'roi', {}, 'peak_id', {}, 'index_before', {}, ...
    'index_after', {}, 'mode', {}, 'created_at', {});
manual_edit_params = struct( ...
    'enabled', logical(params.run_manual_edit), ...
    'min_peak_distance_frames', params.min_peak_distance_frames, ...
...
if params.run_manual_edit
    [peak_table_final, edit_history, peaks_polarity] = edit_voltage_sensitivity_peaks( ...
        voltage_sensitivity, peak_table_detected, peaks_polarity, manual_edit_params);
...
[peaks_index, peaks_amplitude, peaks_polarity] = ...
    voltage_peak_table_to_cells(peak_table_final, nrois, peaks_polarity);
...
peak_results.sensitivity_manually_edited.info = struct( ...
    ...
    'created_at', datetime("now"));
peak_results.accepted_for_events = peak_results.sensitivity_manually_edited;
peak_results.accepted_for_events.info.result_name = 'accepted_for_events';
peak_results.accepted_for_events.info.parent_results = {'sensitivity_manually_edited'};
peak_results.accepted_for_events.info.method = 'accepted_peaks_after_manual_edit';
peak_results.current_result = 'accepted_for_events';

save(fullfile(save_path, '4_voltage_sensitivity_peak_edit_results.mat'), ...
    'peak_table_final', 'edit_history', 'peaks_index', 'peaks_amplitude', ...
    'peaks_polarity', 'manual_edit_params');
save(fullfile(save_path, '5_voltage_accepted_peaks.mat'), ...
    'peaks_index', 'peaks_polarity', 'peaks_amplitude');
save(fullfile(save_path, 'voltage_peak_results.mat'), 'peak_results', '-v7.3');
peak_qc_dir = save_voltage_peak_qc_by_roi( ...
    voltage_sensitivity, peak_table_final, peaks_polarity, save_path);

voltage_results.peak_results = peak_results;
save(voltage_results_path, 'voltage_results', '-v7.3');
fprintf('Voltage peaks saved: %d ROIs | %d accepted peaks\n', ...
    nrois, sum(cellfun(@numel, peaks_index)));
fprintf('Voltage peak QC figures saved to: %s\n', peak_qc_dir);
end

function [is_valid, reason] = validate_saved_voltage_peak_results(voltage_results, expected_nrois, nframes)
is_valid = false;
...
if ~isstruct(voltage_results) || ~isfield(voltage_results, 'peak_results') ...
        ...
        || ~isfield(voltage_results.peak_results, 'accepted_for_events')
    return;
end

accepted = voltage_results.peak_results.accepted_for_events;
required_fields = {'index', 'amplitude', 'polarity'};
if ~isstruct(accepted) || ~isfield(accepted, 'data') || ~isstruct(accepted.data) ...
        || ~all(isfield(accepted.data, required_fields))
    reason = "accepted_for_events data is incomplete";
    return;
end

peaks_index = accepted.data.index;
peaks_amplitude = accepted.data.amplitude;
peaks_polarity = accepted.data.polarity;
if ~iscell(peaks_index) || ~iscell(peaks_amplitude) || ~iscell(peaks_polarity) ...
        || numel(peaks_index) ~= expected_nrois ...
        || numel(peaks_amplitude) ~= expected_nrois ...
        || numel(peaks_polarity) ~= expected_nrois
    reason = "saved peak ROI count does not match the current sensitivity traces";
    return;
end

for roi_idx = 1:expected_nrois
    idx = double(peaks_index{roi_idx}(:));
    amp = double(peaks_amplitude{roi_idx}(:));
    if numel(idx) ~= numel(amp) || any(~isfinite(idx)) ...
            || any(idx ~= round(idx)) || any(idx < 1 | idx > nframes)
        reason = sprintf('saved peak indices/amplitudes are invalid for ROI %d', roi_idx);
        return;
    end
end
...
function [peak_table, peaks_index, peaks_amplitude, peaks_polarity] = ...
    detect_voltage_sensitivity_peaks(traces_sensitivity, frame_rate, params)
nrois = size(traces_sensitivity, 2);
peaks_polarity = cell(1, nrois);
peak_table = empty_voltage_peak_table();
next_peak_id = 1;
...
for roi_idx = 1:nrois
    ...
    polarity = choose_voltage_peak_polarity(trace_i, roi_idx, params);
    peaks_polarity{roi_idx} = polarity;
    plot_trace = trace_i * polarity;
    min_prominence = resolve_voltage_peak_min_prominence(plot_trace, params);
    ...
    peak_table = [peak_table; new_rows]; %#ok<AGROW>
    next_peak_id = next_peak_id + peak_count;
end
[peaks_index, peaks_amplitude, peaks_polarity] = ...
    voltage_peak_table_to_cells(peak_table, nrois, peaks_polarity);
end

function min_prominence = resolve_voltage_peak_min_prominence(plot_trace, params)
if lower(string(params.min_peak_prominence_mode)) == "absolute"
    min_prominence = params.min_peak_prominence;
else
    trace_scale = max(plot_trace, [], 'omitnan') - mean(plot_trace, 'omitnan');
...
function [peak_table, edit_history, peaks_polarity] = edit_voltage_sensitivity_peaks( ...
    traces_sensitivity, peak_table, peaks_polarity, params)
nrois = size(traces_sensitivity, 2);
edit_history = struct('action', {}, 'roi', {}, 'peak_id', {}, 'index_before', {}, ...
    'index_after', {}, 'mode', {}, 'created_at', {});
next_peak_id = 1;
...
while roi_idx <= nrois
    ...
    fig = figure('Name', sprintf('ROI %d voltage sensitivity peak editor', roi_idx));
    set(fig, 'Position', get(0, 'Screensize'));
    while ishandle(fig)
        ax = draw_voltage_peak_editor(fig, traces_sensitivity(:, roi_idx), peak_table, ...
            peaks_polarity{roi_idx}, roi_idx, mode);
        key = wait_for_voltage_peak_editor_key(fig);
        if ~ishandle(fig)
            break;
        end
        switch key
            case "a"
                mode = "add_box";
                [box_position, box_ok] = get_voltage_peak_editor_box(ax);
                if box_ok
                    [peak_table, edit_history, next_peak_id] = add_voltage_peaks_in_box( ...
            ...
            case "d"
                ...
                [box_position, box_ok] = get_voltage_peak_editor_box(ax);
                if box_ok
                    [peak_table, edit_history] = delete_voltage_peaks_in_box( ...
                        traces_sensitivity(:, roi_idx), peak_table, edit_history, ...
                        roi_idx, peaks_polarity{roi_idx}, box_position, mode);
                end
            case "c"
                mode = "clear_current_roi";
                [peak_table, edit_history, cleared_count] = clear_voltage_peaks_for_roi( ...
                    peak_table, edit_history, roi_idx, mode);
                fprintf('ROI %d: cleared %d accepted peaks.\n', roi_idx, cleared_count);
            case "f"
                mode = "flip_polarity";
                old_polarity = normalize_voltage_peak_polarity_value( ...
                    peaks_polarity{roi_idx}, sprintf('ROI %d peak polarity', roi_idx));
                new_polarity = -old_polarity;
                peaks_polarity{roi_idx} = new_polarity;
                peak_table.polarity(peak_table.roi == roi_idx) = new_polarity;
                edit_history(end+1) = make_voltage_peak_edit_history( ...
                    "flip_polarity", roi_idx, NaN, old_polarity, new_polarity, mode); %#ok<AGROW>
                fprintf('ROI %d: peak polarity flipped from %+d to %+d.\n', ...
                    roi_idx, old_polarity, new_polarity);
            case "r"
...
function ax = draw_voltage_peak_editor(fig, trace_i, peak_table, polarity, roi_idx, mode)
polarity = normalize_voltage_peak_polarity_value(polarity, sprintf('ROI %d peak polarity', roi_idx));
figure(fig);
clf(fig);
set(fig, 'KeyPressFcn', @(~, event) set_voltage_peak_editor_key(event, fig));
ax = axes('Parent', fig, 'Units', 'normalized', 'Position', [0.06 0.08 0.91 0.86]);
...
if ~isempty(deleted_idx)
    plot(ax, deleted_idx, trace_i(deleted_idx) * polarity, 'x', ...
        'Color', [0.5 0.5 0.5], 'LineWidth', 1.5);
end
title(ax, sprintf('ROI %d | polarity=%+d | %s | A add, D delete, C clear, F flip, N/Space next, P/Left previous, R reset, Q quit', ...
...
function set_voltage_peak_editor_key(event, fig)
if any(strcmp(event.Key, {'a', 'd', 'c', 'f', 'n', 'space', 'return', 'p', 'leftarrow', 'backspace', 'r', 'q'}))
    fig.UserData.key = event.Key;
end
end

function key = wait_for_voltage_peak_editor_key(fig)
fig.UserData.key = [];
...
function peak_qc_dir = save_voltage_peak_qc_by_roi(traces_sensitivity, peak_table, peaks_polarity, save_path)
peak_qc_dir = fullfile(save_path, '5_voltage_peak_qc_by_roi');
if ~isfolder(peak_qc_dir)
    mkdir(peak_qc_dir);
end
nrois = size(traces_sensitivity, 2);
for roi_idx = 1:nrois
    polarity = 1;
    if numel(peaks_polarity) >= roi_idx && ~isempty(peaks_polarity{roi_idx}) ...
            && isfinite(peaks_polarity{roi_idx})
        polarity = peaks_polarity{roi_idx};
    end
    trace_display = double(traces_sensitivity(:, roi_idx)) * double(polarity);
    accepted_rows = peak_table.roi == roi_idx & peak_table.status == "accepted";
    deleted_rows = peak_table.roi == roi_idx & peak_table.status == "deleted";
    accepted_idx = double(peak_table.index(accepted_rows));
    deleted_idx = double(peak_table.index(deleted_rows));
    accepted_idx = accepted_idx(isfinite(accepted_idx) & accepted_idx >= 1 & accepted_idx <= numel(trace_display));
    deleted_idx = deleted_idx(isfinite(deleted_idx) & deleted_idx >= 1 & deleted_idx <= numel(trace_display));

    fig = figure('Color', 'w', ...
        'Name', sprintf('Voltage Peak QC ROI %03d', roi_idx), ...
        'Visible', 'off', ...
        'Position', [120, 120, 1400, 520]);
    ax = axes(fig);
    plot(ax, trace_display, 'k', 'LineWidth', 0.8);
    hold(ax, 'on');
    if ~isempty(accepted_idx)
        plot(ax, accepted_idx, trace_display(accepted_idx), 'rv', ...
            'MarkerFaceColor', 'r', 'MarkerSize', 5, 'LineWidth', 0.8);
    end
    if ~isempty(deleted_idx)
        plot(ax, deleted_idx, trace_display(deleted_idx), 'x', ...
            'Color', [0.45 0.45 0.45], 'MarkerSize', 6, 'LineWidth', 1.0);
    end
    title(ax, sprintf('ROI %03d Voltage Sensitivity Peak QC | accepted=%d deleted=%d', ...
        roi_idx, numel(accepted_idx), numel(deleted_idx)));
    xlabel(ax, 'Frame');
    ylabel(ax, 'Sensitivity x peak polarity');
    legend(ax, {'Trace', 'Accepted peaks', 'Deleted peaks'}, 'Location', 'best');
    grid(ax, 'on');
    box(ax, 'off');
    fig_file = fullfile(peak_qc_dir, sprintf('roi_%03d_voltage_peak_qc.fig', roi_idx));
    png_file = fullfile(peak_qc_dir, sprintf('roi_%03d_voltage_peak_qc.png', roi_idx));
    save_figure_bundle_preserve_layout(fig, fig_file, png_file);
    close(fig);
end
end

function [box_position, box_ok] = get_voltage_peak_editor_box(ax)
...
try
    ...
    warning('Dual_analysis3:VoltagePeakEditRectangleFailed', ...
        'Rectangle selection failed: %s', ME.message);
end
end

function [peak_table, edit_history, next_peak_id] = add_voltage_peaks_in_box( ...
    trace_i, peak_table, edit_history, next_peak_id, roi_idx, polarity, params, box_position, mode)
plot_trace = trace_i * polarity;
frame_range = max(1, ceil(box_position(1))): ...
    min(numel(trace_i), floor(box_position(1) + box_position(3)));
if isempty(frame_range)
    return;
end
[peak_y, peak_x_rel] = findpeaks(plot_trace(frame_range), ...
    'MinPeakDistance', max(1, round(params.min_peak_distance_frames)));
candidate_indices = frame_range(peak_x_rel);
inside_y = peak_y >= box_position(2) & peak_y <= box_position(2) + box_position(4);
candidate_indices = unique(candidate_indices(inside_y));
existing = peak_table.index(peak_table.roi == roi_idx & peak_table.status == "accepted");

for idx = reshape(candidate_indices, 1, [])
    if ~isempty(existing) && any(abs(existing - idx) < params.min_peak_distance_frames)
        continue;
    end
    new_row = table(next_peak_id, roi_idx, idx, idx / params.frame_rate_hz, polarity, ...
        trace_i(idx), "accepted", "manual_add", NaN, "sensitivity_manually_edited", ...
        datetime("now"), 'VariableNames', peak_table.Properties.VariableNames);
    peak_table = [peak_table; new_row]; %#ok<AGROW>
    edit_history(end+1) = make_voltage_peak_edit_history( ...
        "add_box", roi_idx, next_peak_id, NaN, idx, mode); %#ok<AGROW>
    existing(end+1, 1) = idx; %#ok<AGROW>
    next_peak_id = next_peak_id + 1;
end
end

function [peak_table, edit_history] = delete_voltage_peaks_in_box( ...
    trace_i, peak_table, edit_history, roi_idx, polarity, box_position, mode)
rows = find(peak_table.roi == roi_idx & peak_table.status == "accepted");
if isempty(rows)
    return;
end
peak_x = peak_table.index(rows);
peak_y = trace_i(peak_x) * polarity;
inside = peak_x >= box_position(1) & peak_x <= box_position(1) + box_position(3) ...
    & peak_y >= box_position(2) & peak_y <= box_position(2) + box_position(4);
for row_idx = reshape(rows(inside), 1, [])
    peak_table.status(row_idx) = "deleted";
    edit_history(end+1) = make_voltage_peak_edit_history( ...
        "delete_box", roi_idx, peak_table.peak_id(row_idx), ...
        peak_table.index(row_idx), NaN, mode); %#ok<AGROW>
end
end

function [peak_table, edit_history, cleared_count] = clear_voltage_peaks_for_roi( ...
    peak_table, edit_history, roi_idx, mode)
rows = find(peak_table.roi == roi_idx & peak_table.status == "accepted");
cleared_count = numel(rows);
for row_idx = reshape(rows, 1, [])
    peak_table.status(row_idx) = "deleted";
    edit_history(end+1) = make_voltage_peak_edit_history( ...
        "clear_current_roi", roi_idx, peak_table.peak_id(row_idx), ...
        peak_table.index(row_idx), NaN, mode); %#ok<AGROW>
end
end

function entry = make_voltage_peak_edit_history(action, roi_idx, peak_id, index_before, index_after, mode)
entry = struct( ...
...
function peak_table = empty_voltage_peak_table()
peak_table = table( ...
...
function polarity = choose_voltage_peak_polarity(trace_i, roi_idx, params)
[has_roi_override, roi_polarity] = get_voltage_peak_roi_polarity_override(params.roi_polarity, roi_idx);
if has_roi_override
    polarity = roi_polarity;
    return;
end
...
switch mode
    case {"global", "voltage", "display"}
        polarity = normalize_voltage_peak_polarity_value(params.global_polarity, 'voltage peak global polarity');
    case "positive"
        polarity = 1;
    case "negative"
        polarity = -1;
    ...
    otherwise
        error('Unknown voltage_peak_polarity_mode: %s', mode);
end
end

function [has_override, polarity] = get_voltage_peak_roi_polarity_override(roi_polarity, roi_idx)
has_override = false;
polarity = NaN;
if isempty(roi_polarity)
    return;
end
if iscell(roi_polarity)
    if numel(roi_polarity) < roi_idx || isempty(roi_polarity{roi_idx})
        return;
    end
    value = roi_polarity{roi_idx};
else
    if numel(roi_polarity) < roi_idx || isempty(roi_polarity(roi_idx))
        return;
    end
...
if ~isnumeric(value) || ~isscalar(value)
    error('voltage_peak_roi_polarity for ROI %d must be a finite numeric scalar when provided.', roi_idx);
end
if isnan(value) || value == 0
    return;
end
if ~isfinite(value)
    error('voltage_peak_roi_polarity for ROI %d must be finite when provided.', roi_idx);
end
polarity = normalize_voltage_peak_polarity_value(value, sprintf('voltage_peak_roi_polarity ROI %d', roi_idx));
has_override = true;
end

function polarity = normalize_voltage_peak_polarity_value(value, value_name)
if isempty(value) || ~isnumeric(value) || ~isscalar(value) || ~isfinite(value) || value == 0
    error('%s must be a finite nonzero numeric scalar.', value_name);
end
if value > 0
    polarity = 1;
else
...
function [peaks_index, peaks_amplitude, peaks_polarity] = ...
    voltage_peak_table_to_cells(peak_table, nrois, fallback_polarity)
peaks_index = cell(1, nrois);
peaks_amplitude = cell(1, nrois);
peaks_polarity = cell(1, nrois);
for roi_idx = 1:nrois
    rows_i = peak_table(peak_table.roi == roi_idx & peak_table.status == "accepted", :);
    if height(rows_i) > 0
        ...
        if nargin >= 3 && numel(fallback_polarity) >= roi_idx && ...
...
function stim_results = run_dual_stim_aware_analysis_section( ...
    save_path, ...
    voltage_results, voltage_results_path, ...
    calcium_results, calcium_results_path, ...
    stim_results, stim_results_path, ...
    ...
    freq_voltage, freq_calcium, ...
    voltage_polarity, calcium_polarity, calcium_smoothing_window)
% Shared stimulus-aware analysis used by full, analysis-only, and post-trace paths.
% It keeps grating tuning parameters and stim-shadow summary plotting in one place.
...
if stim_context.supported
    ...
    if stim_windows.supported
        ...
        if stim_windows.is_grating
            if isempty(calcium_stim_metrics_sensitivity)
                error('Grating tuning requires the calcium sensitivity stage.');
            end
            fprintf('Grating tuning response definitions:\n');
            fprintf('  voltage grating/non-grating curves = accepted spike count in paired stim/baseline windows\n');
            fprintf('  calcium grating/non-grating curves = mean sensitivity in paired stim/baseline windows\n');
            fprintf('  calcium sensitivity zero baseline = 0; no stim-minus-baseline subtraction\n');
            fprintf('  DSI/OSI/gDSI/gOSI source = grating-present curves only\n');
            voltage_peaks_index = resolve_voltage_peak_indices(voltage_results);
            voltage_spike_counts = compute_peak_counts_by_trial_windows( ...
                voltage_peaks_index, stim_windows.voltage);
            [voltage_tuning, calcium_tuning, per_roi_tuning] = compute_dual_grating_tuning_by_roi( ...
                voltage_spike_counts.stim, voltage_spike_counts.baseline, ...
                calcium_stim_metrics_sensitivity.stim_mean, ...
                calcium_stim_metrics_sensitivity.baseline_mean, ...
                stim_windows.orientations);
            if strcmpi(string(stim_windows.stim_type), "visualstim_random_grating")
                stim_results.analysis_kind = 'random_grating_tuning';
            else
            ...
            stim_results.tuning = struct( ...
                ...
                'trial_response', struct( ...
                    'voltage_spike_count', voltage_spike_counts, ...
                    'calcium_mean_sensitivity', struct( ...
                        'stim', calcium_stim_metrics_sensitivity.stim_mean, ...
                        'baseline', calcium_stim_metrics_sensitivity.baseline_mean)), ...
                'curve_definition', struct( ...
                    'voltage', "accepted spike count per paired window", ...
                    'calcium', "mean sensitivity per paired window; sensitivity baseline is zero", ...
                    'negative_trial_response_rule', "clip to zero before averaging by direction"), ...
                'dsi_osi_source', "grating_present_curve");
            ...
            save(fullfile(save_path, '7_stim_tuning_summary.mat'), ...
                'voltage_tuning', 'calcium_tuning', 'per_roi_tuning', ...
                'voltage_spike_counts');
            stim_tuning_fig = figure('Color', 'w');
            subplot(2, 2, 1); plot_tuning_population(voltage_tuning, [1, 0, 0], 'Voltage Spike Count', 'Spikes / trial');
            subplot(2, 2, 2); plot_tuning_population(calcium_tuning, [0, 1, 0], 'Calcium Mean Sensitivity', 'Mean sensitivity');
...
function peaks_index = resolve_voltage_peak_indices(voltage_results)
if ~isfield(voltage_results, 'peak_results') || ...
        ~isfield(voltage_results.peak_results, 'accepted_for_events')
    error(['Voltage peak results are unavailable. Run the Voltage Sensitivity ' ...
        'Peak Detection section before grating tuning.']);
end
peaks_index = voltage_results.peak_results.accepted_for_events.data.index;
end

function counts = compute_peak_counts_by_trial_windows(peaks_index, channel_windows)
nrois = numel(peaks_index);
ntrials = size(channel_windows.stim_frames, 1);
counts = struct( ...
    'baseline', zeros(nrois, ntrials), ...
    'stim', zeros(nrois, ntrials), ...
    'baseline_frames', channel_windows.baseline_frames, ...
    'stim_frames', channel_windows.stim_frames);
for roi_idx = 1:nrois
    roi_peaks = double(peaks_index{roi_idx}(:));
    roi_peaks = roi_peaks(isfinite(roi_peaks));
    for trial_idx = 1:ntrials
        base_range = channel_windows.baseline_frames(trial_idx, :);
        stim_range = channel_windows.stim_frames(trial_idx, :);
        counts.baseline(roi_idx, trial_idx) = sum( ...
            roi_peaks >= base_range(1) & roi_peaks <= base_range(2));
        counts.stim(roi_idx, trial_idx) = sum( ...
            roi_peaks >= stim_range(1) & roi_peaks <= stim_range(2));
    end
end
end

function tuning = compute_grating_tuning(trial_response, orientations)
...
for i = 1:nconditions
    ...
    response_by_condition(:, i) = mean(max(trial_response(:, mask), 0), 2, 'omitnan');
end

tuning = struct();
tuning.unique_orientations = unique_orientations(:)';
tuning.trial_response = trial_response;
...
function [voltage_tuning, calcium_tuning, per_roi_tuning] = compute_dual_grating_tuning_by_roi( ...
    voltage_stim_response, voltage_baseline_response, ...
    calcium_stim_response, calcium_baseline_response, orientations)
% The grating-present curves carry DSI/OSI. Paired non-grating curves are
% stored beside them for direct baseline comparison.
nrois = max([size(voltage_stim_response, 1), size(calcium_stim_response, 1)]);
voltage_tuning = attach_nonstim_grating_curve( ...
    compute_grating_tuning(voltage_stim_response, orientations), ...
    compute_grating_tuning(voltage_baseline_response, orientations));
calcium_tuning = attach_nonstim_grating_curve( ...
    compute_grating_tuning(calcium_stim_response, orientations), ...
    compute_grating_tuning(calcium_baseline_response, orientations));
per_roi_tuning = repmat(empty_grating_roi_tuning(), nrois, 1);

for roi_idx = 1:nrois
    voltage_roi_stim = NaN(1, numel(orientations));
    voltage_roi_baseline = NaN(1, numel(orientations));
    calcium_roi_stim = NaN(1, numel(orientations));
    calcium_roi_baseline = NaN(1, numel(orientations));
    if roi_idx <= size(voltage_stim_response, 1)
        voltage_roi_stim = voltage_stim_response(roi_idx, :);
        voltage_roi_baseline = voltage_baseline_response(roi_idx, :);
    end
    if roi_idx <= size(calcium_stim_response, 1)
        calcium_roi_stim = calcium_stim_response(roi_idx, :);
        calcium_roi_baseline = calcium_baseline_response(roi_idx, :);
    end

    per_roi_tuning(roi_idx).roi_index = roi_idx;
    per_roi_tuning(roi_idx).voltage = attach_nonstim_grating_curve( ...
        compute_grating_tuning(voltage_roi_stim, orientations), ...
        compute_grating_tuning(voltage_roi_baseline, orientations));
    per_roi_tuning(roi_idx).calcium = attach_nonstim_grating_curve( ...
        compute_grating_tuning(calcium_roi_stim, orientations), ...
        compute_grating_tuning(calcium_roi_baseline, orientations));
end
end

function tuning = attach_nonstim_grating_curve(tuning, nonstim_tuning)
...
for roi_idx = 1:numel(per_roi_tuning)
    ...
    subplot(2, 2, 1);
    plot_tuning_single(voltage_roi_tuning, [1, 0, 0], sprintf('ROI %03d Voltage Spike Count', roi_entry.roi_index), 'Spikes / trial');
    subplot(2, 2, 2);
    plot_tuning_single(calcium_roi_tuning, [0, 1, 0], sprintf('ROI %03d Calcium Mean Sensitivity', roi_entry.roi_index), 'Mean sensitivity');
    subplot(2, 2, 3);
...
function plot_tuning_single(tuning, line_color, panel_title, y_axis_label)
angles = tuning.unique_orientations(:)';
stim_response = tuning.response_by_condition;
nonstim_response = tuning.nonstim_response_by_condition;
if isempty(stim_response)
    stim_response = NaN(1, numel(angles));
elseif size(stim_response, 1) > 1
    stim_response = stim_response(1, :);
end
if isempty(nonstim_response)
    nonstim_response = NaN(1, numel(angles));
elseif size(nonstim_response, 1) > 1
    nonstim_response = nonstim_response(1, :);
end
plot(angles, stim_response, '-o', 'Color', line_color, ...
    'MarkerFaceColor', line_color, 'LineWidth', 1.5);
hold on;
plot(angles, nonstim_response, '--o', 'Color', line_color * 0.55 + 0.45, ...
...
function plot_tuning_population(tuning, line_color, panel_title, y_axis_label)
angles = tuning.unique_orientations;
stim_response = tuning.response_by_condition;
nonstim_response = tuning.nonstim_response_by_condition;
stim_mean = mean(stim_response, 1, 'omitnan');
stim_sem = std(stim_response, 0, 1, 'omitnan') ./ sqrt(max(1, sum(isfinite(stim_response), 1)));
nonstim_mean = mean(nonstim_response, 1, 'omitnan');
nonstim_sem = std(nonstim_response, 0, 1, 'omitnan') ./ sqrt(max(1, sum(isfinite(nonstim_response), 1)));
errorbar(angles, stim_mean, stim_sem, '-o', 'Color', line_color, ...
    'MarkerFaceColor', line_color, 'LineWidth', 1.5);
hold on;
errorbar(angles, nonstim_mean, nonstim_sem, '--o', ...
    'Color', line_color * 0.55 + 0.45, 'MarkerFaceColor', 'w', 'LineWidth', 1.2);
xlabel('Direction (deg)');
ylabel(y_axis_label);
...
% Build the dual-channel trace comparison package for one metric type
% (currently sensitivity or SNR). Quad, deconvolution/integral, and
% correlation outputs are intentionally not generated here.
...
fprintf('Dual %s trace comparison saved | overlap=%s\n', metric_name, overlap_png);
...
function [dual_info, voltage_results, calcium_results, dual_results, stim_results, stim_context, stim_windows] = ...
    load_saved_analysis_only_context(reuse_results_path, execution_plan)
% Load the standard saved containers. The atomic initializer decides which
% section-owned fields are copied into the new output folder.
if strlength(string(reuse_results_path)) == 0
    error('A reuse plan requires dual3_control.source_results_path.');
end
...
voltage_peak_results_path = fullfile(reuse_results_path, 'voltage_peak_results.mat');
if (~isfield(voltage_results, 'peak_results') || isempty(voltage_results.peak_results)) ...
        && isfile(voltage_peak_results_path)
    voltage_results.peak_results = load_required_struct(voltage_peak_results_path, 'peak_results');
    fprintf('Loaded saved peak_results from standalone voltage_peak_results.mat.\n');
end
needs_dual_results = any([execution_plan.roi, execution_plan.visualization, ...
    execution_plan.comparison] == "reuse");
if needs_dual_results && isfile(dual_results_path)
    dual_results = load_required_struct(dual_results_path, 'dual_results');
else
    dual_results = struct();
end
if execution_plan.stim == "reuse" && isfile(stim_results_path)
...
fprintf('Reuse source loaded according to the section plan; skipped section results were not loaded.\n');
end

function tf = paths_equal_dual(path_a, path_b)
path_a = regexprep(strrep(strtrim(char(string(path_a))), '/', '\'), '[\\]+$', '');
path_b = regexprep(strrep(strtrim(char(string(path_b))), '/', '\'), '[\\]+$', '');
tf = ~isempty(path_a) && ~isempty(path_b) && strcmpi(path_a, path_b);
end

function [dual_info, voltage_results, calcium_results, dual_results, stim_results, time_frequency_results] = ...
    initialize_atomic_reuse_output( ...
    save_path, source_path, execution_plan, workflow_control, ...
    source_dual_info, source_voltage_results, source_calcium_results, ...
    source_dual_results, source_stim_results)
% Build the new result folder from empty standard containers. Each reuse
% action imports only the fields owned by that section, preserving the
% existing MAT variable names and result struct formats.
dual_info = source_dual_info;
dual_info.source_save_path = string(source_path);
dual_info.save_path = char(string(save_path));
dual_info.workflow_control = workflow_control;
dual_info.atomic_reuse_created_at = datetime("now");

voltage_results = struct( ...
    'movie_info', source_voltage_results.movie_info, ...
    'trace_results', struct());
calcium_results = struct( ...
    'movie_info', source_calcium_results.movie_info, ...
    'trace_results', struct());
voltage_results.movie_info = remove_struct_field_if_present_dual( ...
    voltage_results.movie_info, 'motion');
calcium_results.movie_info = remove_struct_field_if_present_dual( ...
    calcium_results.movie_info, 'motion');
if execution_plan.registration ~= "reuse"
    voltage_results.movie_info = remove_struct_field_if_present_dual( ...
        voltage_results.movie_info, 'channel_registration');
    calcium_results.movie_info = remove_struct_field_if_present_dual( ...
        calcium_results.movie_info, 'channel_registration');
end
...
if execution_plan.roi == "reuse"
    if ~isfield(source_dual_results, 'registration')
        error('roi="reuse" requires dual_results.registration in source_results_path.');
    end
    dual_results.registration = source_dual_results.registration;
    if isfield(source_dual_results, 'maps')
        dual_results.maps = source_dual_results.maps;
    end
    source_roi_file = fullfile(source_path, '1_dual_roi_results.mat');
    if ~isfile(source_roi_file)
        error('roi="reuse" requires source file: %s', source_roi_file);
    end
    destination_roi_file = fullfile(save_path, '1_dual_roi_results.mat');
    copyfile(source_roi_file, destination_roi_file);
    if isfield(dual_results.registration, 'info')
        dual_results.registration.info.source_roi_file = string(source_roi_file);
        dual_results.registration.info.roi_file = destination_roi_file;
    end
end
...
if execution_plan.trace == "reuse"
    if ~isfield(source_voltage_results, 'trace_results') ...
            || ~isfield(source_calcium_results, 'trace_results')
        error('trace="reuse" requires trace_results in both saved channel result files.');
    end
    voltage_results.trace_results = source_voltage_results.trace_results;
    calcium_results.trace_results = source_calcium_results.trace_results;
end

if execution_plan.peak == "reuse"
    if ~isfield(source_voltage_results, 'peak_results')
        error('peak="reuse" requires voltage_results.peak_results or voltage_peak_results.mat.');
    end
    voltage_results.peak_results = source_voltage_results.peak_results;
    expected_nrois = size(fetch_trace_stage(voltage_results, 'sensitivity'), 2);
    nframes = size(fetch_trace_stage(voltage_results, 'sensitivity'), 1);
    [valid_peaks, peak_reason] = validate_saved_voltage_peak_results( ...
        voltage_results, expected_nrois, nframes);
    if ~valid_peaks
        error('peak="reuse" validation failed: %s', peak_reason);
    end
    copy_atomic_peak_files_dual(source_path, save_path, voltage_results.peak_results);
end

if execution_plan.visualization == "reuse"
    if ~isfield(source_dual_results, 'visualizations')
        error('visualization="reuse" requires dual_results.visualizations.');
    end
    dual_results.visualizations = source_dual_results.visualizations;
end
if execution_plan.comparison == "reuse"
    if ~isfield(source_dual_results, 'comparison')
        error('comparison="reuse" requires dual_results.comparison.');
    end
    dual_results.comparison = source_dual_results.comparison;
end
if execution_plan.stim == "reuse"
    if isempty(fieldnames(source_stim_results))
        error('stim="reuse" requires stim_results.mat.');
    end
    stim_results = source_stim_results;
end
if execution_plan.frequency == "reuse"
    source_frequency_file = fullfile(source_path, '8_time_frequency_results.mat');
    time_frequency_results = load_required_struct( ...
        source_frequency_file, 'time_frequency_results');
    save(fullfile(save_path, '8_time_frequency_results.mat'), ...
        'time_frequency_results', '-v7.3');
end
...
fprintf('Atomic reuse containers initialized without copying skipped section results.\n');
end

function s = remove_struct_field_if_present_dual(s, field_name)
if isstruct(s) && isfield(s, field_name)
    s = rmfield(s, field_name);
end
end

function copy_atomic_peak_files_dual(source_path, save_path, peak_results)
accepted_data = peak_results.accepted_for_events.data;
peaks_index = accepted_data.index;
peaks_amplitude = accepted_data.amplitude;
peaks_polarity = accepted_data.polarity;
...
save(fullfile(save_path, 'voltage_peak_results.mat'), 'peak_results', '-v7.3');
optional_files = { ...
    '4_voltage_sensitivity_peak_detect_results.mat', ...
    '4_voltage_sensitivity_peak_edit_results.mat'};
for idx = 1:numel(optional_files)
    source_file = fullfile(source_path, optional_files{idx});
    if isfile(source_file)
        copyfile(source_file, fullfile(save_path, optional_files{idx}));
    end
end
end
...
explicit_dual_results = struct( ...
    ...
    'result_files', struct( ...
        ...
        'voltage_results', voltage_results_path, ...
        'calcium_results', calcium_results_path, ...
        'voltage_peak_results', fullfile(save_path, 'voltage_peak_results.mat'), ...
        'dual_results', dual_results_path, ...
        'stim_results', stim_results_path, ...
        'time_frequency_results', time_frequency_results_path), ...
