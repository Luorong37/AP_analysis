...
movie = double(movie);
movie_ca = double(movie_ca);
colors = lines(100);

[image_height, image_width] = resolve_dual_image_size(movie, nrows, ncols);
mean_voltage = reshape(mean(movie, 2), image_height, image_width);
mean_calcium = reshape(mean(movie_ca, 2), image_height, image_width);
...
for roi_idx = 1:nrois
    ...
    traces(:, end+1) = mean(movie(mask_v(:), :), 1)'; %#ok<AGROW>
    traces_ca(:, end+1) = mean(movie_ca(mask_c(:), :), 1)'; %#ok<AGROW>
